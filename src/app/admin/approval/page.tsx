...
function JobCard({ job, type, onApprove, onReject, onApply }: any) {
  return (
    <div className="bg-white rounded shadow p-4 flex flex-col items-center">
      ...
      <div className="text-sm text-gray-600 text-center mb-2">{job.company}</div>
      <div className="flex gap-2 mb-2">
        <button className="bg-green-500 text-white px-3 py-1 rounded" onClick={() => onApprove(job.id, type)}>Duyệt</button>
        <button className="bg-red-500 text-white px-3 py-1 rounded" onClick={() => onReject(job.id, type)}>Từ chối</button>
      </div>
      <button className="bg-blue-500 text-white px-3 py-1 rounded" onClick={() => onApply(job, type)}>Ứng tuyển</button>
    </div>
  );
}
...
async function updateJobStatus(id: string, type: "job" | "newjob", status: "approved" | "rejected") {
  const url = type === "job"
    ? `/api/admin/jobs/${id}`
    : `/api/admin/newjobs/${id}`;
  await fetch(url, {
    method: "PATCH",
...
export default function ApprovalPage() {
  ...
  const [applyModal, setApplyModal] = useState<{open: boolean, job: any, type: string}>({open: false, job: null, type: ''});
  const [applyLoading, setApplyLoading] = useState(false);

  useEffect(() => {
    setLoading(true);
    Promise.all([fetchPendingJobs(), fetchPendingNewJobs()]).then(([jobs, newJobs]) => {
      setJobs(jobs);
      setNewJobs(newJobs);
      setLoading(false);
    });
  ...
  const handleAction = async (id: string, type: "job" | "newjob", status: "approved" | "rejected") => {
  ...
  const handleApplySubmit = async (e: any) => {
    ...
    const data = {
      ...
      jobId: applyModal.type === 'job' ? applyModal.job.id : undefined,
      hiringId: undefined,
    };
    // Nếu là newjob thì lưu vào jobId luôn (API hỗ trợ cả job/newjob)
    if (applyModal.type === 'newjob') data.jobId = applyModal.job.id;
      ...
      {loading ? <div>Đang tải...</div> : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            {jobs.map(job => (
              <JobCard key={job.id} job={job} type="job" onApprove={(id: string) => handleAction(id, 'job', 'approved')} onReject={(id: string) => handleAction(id, 'job', 'rejected')} onApply={handleApply} />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {newJobs.map(job => (
              <JobCard key={job.id} job={job} type="newjob" onApprove={(id: string) => handleAction(id, 'newjob', 'approved')} onReject={(id: string) => handleAction(id, 'newjob', 'rejected')} onApply={handleApply} />
            ))}
          </div>
        </>
      )}
      <ApplyModal open={applyModal.open} onClose={() => setApplyModal({open: false, job: null, type: ''})} onSubmit={handleApplySubmit} job={applyModal.job} />
    </div>
  );
} 