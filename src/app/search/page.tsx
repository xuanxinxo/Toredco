...
function SearchContent() {
  ...
  const fetchJobs = useCallback(
    async (pageNum: number, searchTerm: string, locationTerm: string) => {
      try {
        ...
        const apiUrl = `/api/jobs/fast?${params.toString()}`;
        ...
        // Handle case where the API returns jobs directly or in a jobs property
        const jobsData = Array.isArray(data) ? data : (data.jobs || []);
        
        if (!Array.isArray(jobsData)) {
          throw new Error("Dữ liệu không hợp lệ từ máy chủ: phản hồi không phải mảng");
        }

        // Update jobs state
        setJobs(prevJobs => 
          isInitialLoad ? jobsData : [...prevJobs, ...jobsData]
        );
        
        // Handle pagination - check if we've reached the end of results
        // If we get fewer items than requested, we've reached the end
        const hasMoreData = jobsData.length >= 10; // Assuming 10 is the page size
    ...
    const currentObserver = new IntersectionObserver(
      (entries) => {
        ...
        if (target.isIntersecting) {
          fetchJobs(page + 1, jobTitle, province);
        }
      },
      {
        root: null,
  ...
  }, [fetchJobs, hasMore, jobTitle, loading, loadingMore, page, province]);
